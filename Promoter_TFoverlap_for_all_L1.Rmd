...
TF.files = mapping %>% filter(stage == STAGE) %>% pull(path)
names(TF.files) = mapping %>% filter(stage == STAGE) %>% pull(name)
intestine_enriched = geneNamesToWBID[unique(names(TF.files)),]

if (STAGE == "LE") {
   intestine_enriched$enriched = intestine_enriched$WBGeneID %in% LE.enriched$WBGeneID
   promoters_gr = makeGRangesFromDataFrame(LE.enriched, keep.extra.columns = T, seqinfo=Seqinfo(genome="ce11"))
}else if (STAGE == "L1"){
   intestine_enriched$enriched = intestine_enriched$WBGeneID %in% L1.enriched$WBGeneID
   promoters_gr = makeGRangesFromDataFrame(L1.enriched, keep.extra.columns = T, seqinfo=Seqinfo(genome="ce11"))
}else if (STAGE == "L3"){
  intestine_enriched$enriched = intestine_enriched$WBGeneID %in% L3.enriched$WBGeneID
  promoters_gr = makeGRangesFromDataFrame(L3.enriched, keep.extra.columns = T, seqinfo=Seqinfo(genome="ce11"))
}
...
```{r hclust-for-screeplot, eval=FALSE}
...
int.enriched = data.frame(enriched=factor( ifelse(intestine_enriched[,'enriched'],'yes','no'))) 
