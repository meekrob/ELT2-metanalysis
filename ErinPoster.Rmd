...
sort_hc_by_svd = function(hc, mat, which.vec=1){
    sv = svd(t(mat))$v[,which.vec]
    dend = reorder(as.dendrogram(hc), wts = sv^2)
    sorted.hc = as.hclust(dend)
    sorted.hc$distance.method = hc$distance.method
    sorted.hc$method = hc$method
    sorted.hc
}
...
embryo.DBPeakOverlapsIntestinePresent = intestinePresentDB %>% 
  filter( is.na(intestinePresentDB$L1) & 
          is.na(intestinePresentDB$L3)) %>% 
  inner_join(DBPeakOverlaps, by = "name")

embryo.DBPeakOverlapsIntestinePresent %<>% column_to_rownames("name") %>% 
  select(-WBID, -embryo, -L1, -L3)
...
embryo.mx = ifelse(naOrZero(embryo.DBPeakOverlapsIntestinePresent), 0, 1)
```
...
embryo.fractionZero = sum(embryo.mx == 0) / length(embryo.mx)
cat("fraction of zero embryo cells:", embryo.fractionZero,"\n")
```
...
rs = rowSums(embryo.mx)
cs = colSums(embryo.mx)
cat("Embryo: The following genes are bound in over 90% of the ChIP-seq experiments:")
rownames(embryo.mx)[rs > .9*ncol(embryo.mx)]
```
...
system.time({hc.mx.cols = hclust(eu.mx.col, method = "average")})
  ...
  # 0.005   0.001   0.006
```
```{r embryo euclidean distance}
system.time({embryo.eu.mx.row = dist(embryo.mx)})
  #  user  system elapsed 
  # 0.609   0.002   0.611
system.time({embryo.eu.mx.col = dist(t(embryo.mx))})
  #  user  system elapsed 
  # 0.230   0.002   0.231 
system.time({embryo.hc.mx.rows = hclust(embryo.eu.mx.row, method = "average")})
  #  user  system elapsed 
  # 0.021   0.002   0.025 
system.time({embryo.hc.mx.cols = hclust(embryo.eu.mx.col, method = "average")})
  #  user  system elapsed 
  # 0.005   0.001   0.006
```
```{r embryo square heatmap}
embryo.eu.mx.col.mat = as.matrix(embryo.eu.mx.col)
hm=Heatmap(embryo.eu.mx.col.mat, 
           cluster_rows = embryo.hc.mx.cols, 
           cluster_columns = embryo.hc.mx.cols,
           heatmap_legend_param = list(title = "euclidean distance"))
```

```{r save embryo sq heatmap}
pdf(file=format(Sys.time(), "plots/embryo-square-heatmap-%Y-%m-%d-%H-%M.pdf"), 
    width=50, height=50)
hm
dev.off()
```
...
system.time({hc.mx.cosine.cols = hclust(cos.mx.col, method = "average")})
  ...
  # 0.005   0.000   0.006 
```
```{r embryo cosine distance}
system.time({embryo.cos.mx.row = cosine.dist(embryo.mx)})
  #    user  system elapsed 
  # 1.099   0.016   1.119
system.time({embryo.cos.mx.col = cosine.dist(t(embryo.mx))})
  #  user  system elapsed 
  # 0.287   0.002   0.290 

system.time({embryo.hc.mx.cosine.rows = hclust(embryo.cos.mx.row, method = "average")})
  # user  system elapsed 
  # 0.019   0.000   0.020 
# system.time({embryo.hc.mx.cosine.cols = hclust(embryo.cos.mx.col, method = "average")})
# Error in hclust(embryo.cos.mx.col, method = "average") : 
...
```
```{r embryo scree}
plot(1:length(embryo.hc.mx.rows$height),rev(embryo.hc.mx.rows$height),  xlim=c(0,50), main="embryo scree for euclidean rows")
plot(1:length(embryo.hc.mx.cols$height),rev(embryo.hc.mx.cols$height),  xlim=c(0,50), main="embryo scree for euclidean columns")

plot(1:length(embryo.hc.mx.cosine.rows$height),rev(embryo.hc.mx.cosine.rows$height),  xlim=c(0,50), main="embryo scree for cosine rows")
#plot(1:length(hc.mx.cosine.cols$height),rev(hc.mx.cosine.cols$height),  xlim=c(0,50), main="scree for cosine columns")
```

```{r all cos and euclidean}

hmColors = c('#FFFFCC',  '#68001C')

hm.euclidean = Heatmap(mx, 
        cluster_rows = hc.mx.rows, 
        #cluster_columns = sort_hc_by_svd(hc.mx.cols, t(mx), 2), 
...
hm.cosine = Heatmap(mx, 
        ...
        #cluster_columns = sort_hc_by_svd(hc.mx.cosine.cols,t(mx), 2), 
...
pdf(file=format(Sys.time(), "plots/all-heatmap-euc-%Y-%m-%d-%H-%M.pdf"), 
    width=60, height=20)
hm.euclidean
dev.off()

pdf(file=format(Sys.time(), "plots/all-heatmap-cos-%Y-%m-%d-%H-%M.pdf"), 
    width=60, height=20)
hm.cosine
dev.off()
```
...
embryo.hm.euclidean = Heatmap(embryo.mx, 
        cluster_rows = embryo.hc.mx.rows, 
        cluster_columns = embryo.hc.mx.cols, 
        heatmap_legend_param = list(title = "euclidean\nTF bound/unbound"),
        show_heatmap_legend = FALSE,
        show_row_names = FALSE, 
        ...
        column_names_side = "top",
        use_raster = T)
pdf(file=format(Sys.time(), "plots/embryo-heatmap-euc-%Y-%m-%d.pdf"), 
    width=60, height=20)
hm.euclidean
dev.off()
...
```{r embryo heatmap-ELT-2}
embryo.ELT2bound = embryo.mx[,'elt-2_LE_1']

embryo.mx.noELT2 = embryo.mx[,-which(colnames(embryo.mx) == 'elt-2_LE_1')]

dex = dist(t(embryo.mx.noELT2))
hc_cols = hclust(dex, method = "average")


num_binding = colSums(embryo.mx)[-which(colnames(embryo.mx) == 'elt-2_LE_1')]
q_num_binding = quantile(num_binding)
ecdf_num_binding = ecdf(num_binding)(num_binding)

#hc_cols_svd = sort_hc_by_svd(hc_cols, t(embryo.mx.noELT2), 2)
#min_binding_threshold = q_num_binding['50%']
min_binding_threshold = 0


system.time({
hm.embryo.elt2.binding = Heatmap(embryo.mx.noELT2[,num_binding>min_binding_threshold], 
        clustering_method_rows = "average",
        cluster_columns = sort_hc_by_svd(hclust(dist(t(embryo.mx.noELT2[,num_binding>min_binding_threshold])), 
                                                method = "average"), 
                                         t(embryo.mx.noELT2[,num_binding>min_binding_threshold]), 2),
        heatmap_legend_param = list(title = "euclidean\nTF bound/unbound"),
        show_heatmap_legend = FALSE,
        show_row_names = FALSE, 
        ...
        show_column_dend = T,
        col = hmColors,
        row_split = ifelse(embryo.ELT2bound, "ELT-2 bound", "ELT-2 never bound"),
        column_split = 7,
        column_names_side = "top",
        use_raster = T,
        
        ### comment out this annotation to hide num_binding highlights (but restore show_column_names)
        # top_annotation = HeatmapAnnotation(
        #   labs = anno_text(colnames(embryo.mx.noELT2)[num_binding>min_binding_threshold],
        #                    gp = gpar(col=ifelse(num_binding>q_num_binding['50%'], 
        #                                         'black', 
        #                                         'grey'))),
        #   numGenesBound = anno_barplot(num_binding[num_binding>min_binding_threshold]))
        ###
        )
})

pdf(file=format(Sys.time(), "plots/heatmap-embryo-elt2-binding-%Y-%m-%d-%H-%M.pdf"), 
    width=60, height=20, family = "ArialMT")
hm.embryo.elt2.binding
dev.off()
```

```{r embryo heatmap subset}
embryo.ELT2bound = embryo.mx[,'elt-2_LE_1']
embryo.mx.noELT2 = embryo.mx[,-which(colnames(embryo.mx) == 'elt-2_LE_1')]

### deal with some long column names
cc = colnames(embryo.mx.noELT2)
cc[cc == 'W03F9.2_L4+yAd_1'] = 'W03F9.2_L4_1'
names_to_plot = sub('_[0-9]+$', '',cc) # remove last _[0-9]. 
                                       # The names are not unique
                                       # so must be plotted separately (can't be non-unique colnames)



# get the spread of gene binding totals
num_binding = colSums(embryo.mx)[-which(colnames(embryo.mx) == 'elt-2_LE_1')]
q_num_binding = quantile(num_binding)
ecdf_num_binding = ecdf(num_binding)(num_binding)
# apply threshold
min_binding_threshold = q_num_binding['75%']
reduced.mx = embryo.mx.noELT2[,num_binding>min_binding_threshold]
# compute dist and clustering
dex = dist(t(reduced.mx))

hc_cols = sort_hc_by_svd(hclust(dex, method = "complete"), 
                         t(reduced.mx), 2)
```
...
lvls=c("ELT-2 embryo bound", "ELT-2 not bound in embryo")
elt2_row_order = factor(ifelse(embryo.ELT2bound, lvls[1],lvls[2]),
                        levels=rev(lvls))

# plot primary reduced heatmap
hm.embryo.elt2.binding.subset = 
  Heatmap(name = "transcription factor binding profile", reduced.mx, 
        clustering_method_rows = "average",
        cluster_columns = hc_cols,
        heatmap_legend_param = list(title = "TF bound/unbound"),
        ...
        show_column_dend = T,
        col = hmColors,
        row_split = elt2_row_order,
        row_title="Intestine present genes",
        column_title="TFs binding >164 intestine present genes",
        column_split = 7,
        column_names_side = "top",
        height=unit(7, "inches"),
        top_annotation = HeatmapAnnotation(
          labs = anno_text(names_to_plot[num_binding>min_binding_threshold],
                           just=0, location=0, gp = gpar(fontfamily="Courier"))
          
          ),
        use_raster = T
        )
        # print(
        #   draw(hm.embryo.elt2.binding.subset, 
        #        column_title="TFs binding >164 intestine present genes",
        #        column_title_gp = gpar(fontsize = 18, fontfamily="ArialMT"))
        #   )
pdf(file=format(Sys.time(), "plots/subset-embryo-elt2-binding-%Y-%m-%d-%H-%M.pdf"), 
    width=20, height=20, family = "ArialMT")
...
```{r}
# square
dex.asmx = as.matrix(dex)
hm_reduced_square_embryo = Heatmap(column_title = "TF similarity", 
                                   row_title = "TF similarity", 
                                   dex.asmx,
           cluster_rows = hc_cols, 
           cluster_columns = hc_cols,
           column_split = 7,
           row_split = 7,
           show_row_names = FALSE, 
           show_column_names = F,
           height=unit(10, "inches"),
            top_annotation = HeatmapAnnotation(
          labs = anno_text(names_to_plot[num_binding>min_binding_threshold],
                           just=0, location=0, gp = gpar(fontfamily="Courier"))
           ...
           heatmap_legend_param = list(title = "euclidean distance"))
pdf(file=format(Sys.time(), "plots/subset-embryo-square-%Y-%m-%d-%H-%M.pdf"), 
    width=20, height=25, family = "ArialMT")
draw(
hm.embryo.elt2.binding.subset %v% hm_reduced_square_embryo,
column_title="modENCODE TF binding of intestine present genes")
dev.off()
```

```{r}
euclidean.dex = dist(t(reduced.mx))
cos.dex = cosine.dist(t(reduced.mx))
hc_cosine = sort_hc_by_svd(hclust(cos.dex, method = "ward.D2"), 
                         t(reduced.mx), 2)
hc_euclidean = sort_hc_by_svd(hclust(euclidean.dex, method = "ward.D2"), 
                         t(reduced.mx), 2)
# square
cos.dmx = as.matrix(cos.dex)
scaled.euclidean.dex = ((euclidean.dex) - min(euclidean.dex))/diff(range(euclidean.dex))
#euclidean.dmx = as.matrix(scaled.euclidean.dex) # make euclidean.dmx from 0 to 1
euclidean.dmx = as.matrix(euclidean.dex)

# smashed: lower tri = euclidean, upper tri = cos
# smashed.dmx = cos.dmx
# smashed.dmx[lower.tri(smashed.dmx)] = euclidean.dmx[lower.tri(euclidean.dmx)]

hm_reduced_square_embryo = Heatmap(euclidean.dmx,
           cluster_rows = hc_euclidean,
           cluster_columns = hc_euclidean,
           heatmap_legend_param = list(title = "euclidean distance"),
           column_names_side = "top",
           row_names_side = "left",
           column_split = 7,
           row_split = 7)
pdf(file=format(Sys.time(), "plots/subset-embryo-square-%Y-%m-%d-%H-%M.pdf"), 
    width=60, height=60, family = "ArialMT")
hm_reduced_square_embryo
dev.off()
```
...
pdf(file=format(Sys.time(), "plots/heatmap-elt2-binding-%Y-%m-%d-%H-%M.pdf"), 
    width=60, height=20)
hm.elt2.binding
dev.off()
```
...
pdf(file=format(Sys.time(), "plots/heatmap-states-labeled-%Y-%m-%d-%H-%M.pdf"), 
    width=60, height=20)
hm.states
dev.off()
```
...
pdf(file=format(Sys.time(), "plots/heatmap-states-clustered-%Y-%m-%d-%H-%M.pdf"), 
    width=50, height=20)
hm.states.clustered
dev.off()
