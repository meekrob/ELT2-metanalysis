...
```{r}
heatmap(mx.filt[rowSums(mx.ind)>2,], scale="none")
```
...
heatmap(mx.ind,scale="none")
heatmap(mx.ind[rowSums(mx.ind)>2,], scale="none")
```

```{r svd}
SVD = svd(mx.ind[rowSums(mx.ind)>2,])
u = SVD$u
d <- diag(SVD$d)
v = SVD$v
```

```{r try-different-depths}
mx.ind = mx.ind[rowSums(mx.ind)>2,]
heatmap(mx.ind, scale="none")
hist(mx.ind)
...
ls <- us %*% ds %*% t(vs)
colnames(ls) <- colnames(mx.ind)
heatmap(ls, scale="none")
hist(ls)
