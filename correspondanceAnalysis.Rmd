...
bigDF.din = bigDF %>% filter(!is.na(din.status)) %>% select(din.status.description, embryo_int_exp, L1_int_exp, L3_int_exp, LE_bound:ZTF.7) %>% select(-i, -score, -., -core.start, -core.end)
...
agg1[,c('CEH.9','MML.1', 'FKH.2','NHR.67')]
#                      CEH.9 MML.1 FKH.2 NHR.67
# down_ELT2_minus          0     0     0      0
# unchanged_ELT2_minus     0     0     0      0
# up_ELT2_minus            1     1     1      1
x %>% filter(CEH.9 > 0 | MML.1 > 0 | FKH.2 > 0 | NHR.67 > 0) # get the gene name on here
