...
LE.depleted.TF.Bound.count = countOverlaps(makeGRangesFromDataFrame(LE.depleted, keep.extra.columns = T), all.grs[["fname"]]) %>% sum()
data.frame(
        tf.bound=factorname,
        number_times_bound=LE.depleted.TF.Bound.count, 
        )
) #loop ends
binding_count.df=do.call("rbind",all.grs)
binding_count.df
```
