...
library(pheatmap)
library(proxy) # for dist() with a method argument that can be a function
...
```{r apply-cos-dist}
system.time({
dist_cos_rows = dist(mx.ind, method=distance.cosine)
dist_cos_cols = dist(t(mx.ind), method=distance.cosine)})
```
